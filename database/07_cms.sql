...
--   3. f_link_type 决定跳转目标 (url / product / activity / hospital / page)
...
COMMENT ON COLUMN public.t_banner.f_link_type   IS '跳转目标类型 (白名单): url / product / activity / hospital / page';
COMMENT ON COLUMN public.t_banner.f_link_url    IS '跳转 URL 或内部路径 (具体语义由 f_link_type 决定)';
...
COMMENT ON COLUMN public.t_banner.f_status_user IS 'FK -> public.t_status(f_id) | defined in 01_enums.sql | 软删';
...
COMMENT ON COLUMN public.t_activity.f_status_user      IS 'FK -> public.t_status(f_id) | defined in 01_enums.sql | 软删';
...
COMMENT ON COLUMN public.t_landing_page.f_status_user     IS 'FK -> public.t_status(f_id) | defined in 01_enums.sql | 软删';
