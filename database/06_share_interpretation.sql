-- ============================================================
-- PetChat (灵犀宠语) / 6. 分享与解读 / Share & Interpretation
-- ============================================================
-- Version: 4.0.0
-- Created: 2026-06-17
...
--
-- 依赖:
--   01_enums.sql       (t_share_type, t_share_medium, t_status, t_lang)
...
CREATE TABLE public.t_share_record (
    ...
    f_report_type     VARCHAR(32) NOT NULL,
    f_share_type_id   BIGINT  NOT NULL,
    f_share_medium_id BIGINT  NOT NULL,
    f_share_url       VARCHAR(512) NOT NULL DEFAULT '',
    ...
    CONSTRAINT fk_t_share_medium  FOREIGN KEY (f_share_medium_id) REFERENCES public.t_share_medium(f_id) ON DELETE NO ACTION,
...
ALTER TABLE public.t_share_record ADD COLUMN IF NOT EXISTS f_report_type VARCHAR(32) NOT NULL DEFAULT '';
...
COMMENT ON COLUMN public.t_share_record.f_report_type     IS '报告类型 (冗余, 便于查询): emotion / health / human_pet_risk / personality | 冗余源: t_report_type.f_code (in 01_enums.sql)';
COMMENT ON COLUMN public.t_share_record.f_share_type_id   IS 'FK -> public.t_share_type(f_id) | defined in 01_enums.sql | 分享内容类型';
COMMENT ON COLUMN public.t_share_record.f_share_medium_id IS 'FK -> public.t_share_medium(f_id) | defined in 01_enums.sql | 分享渠道 (微信/朋友圈/...)';
...
COMMENT ON COLUMN public.t_interpretation_voice.f_report_type     IS '报告类型: emotion / health / human_pet_risk / personality | 冗余源: t_report_type.f_code (in 01_enums.sql)';
