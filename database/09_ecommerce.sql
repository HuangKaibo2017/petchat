...
--      格式: {"zh-CN": "...", "en-US": "...", "ja-JP": "..."}; pgroonga 索引覆盖全语言搜索
...
CREATE INDEX idx_t_product_category_active ON public.t_product_category(f_deleted) WHERE f_deleted = 0;
...
-- SPU i18n 等值/排序: B-tree 表达式索引 (常用 locale)
CREATE INDEX IF NOT EXISTS idx_t_product_spu_name_en       ON public.t_product_spu ((f_name->>'en-US'));
CREATE INDEX IF NOT EXISTS idx_t_product_spu_name_zh       ON public.t_product_spu ((f_name->>'zh-CN'));
-- SPU 全文搜索: pgroonga (单列索引; multicolumn JSONB 索引 pgroonga 暂不支持)
-- 前提: CREATE EXTENSION pgroonga WITH SCHEMA extensions; (见 00_extensions.sql)
CREATE INDEX IF NOT EXISTS idx_t_product_spu_pgroonga
    ON public.t_product_spu
    USING pgroonga (f_name);
...
-- t_cart 无软删字段, 用 CASCADE 清理 (用户或商品删除时), 不建 active 部分索引
CREATE INDEX IF NOT EXISTS idx_t_cart_user                 ON public.t_cart(f_user_id);
