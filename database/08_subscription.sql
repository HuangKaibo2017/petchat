...
CREATE TABLE public.t_user_subscription (
    ...
    f_order_at            TIMESTAMPTZ NOT NULL,
    f_activated_at        TIMESTAMPTZ NOT NULL,
    ...
    CONSTRAINT ck_t_us_dates CHECK (f_expire_at > f_activated_at),
...
COMMENT ON COLUMN public.t_user_subscription.f_order_at           IS '下单时间 (UTC)';
COMMENT ON COLUMN public.t_user_subscription.f_activated_at       IS '生效时间 (UTC) | 约束: < f_expire_at';
COMMENT ON COLUMN public.t_user_subscription.f_expire_at          IS '过期时间 (UTC) | 约束: > f_activated_at';
