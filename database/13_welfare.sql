-- ============================================================
-- PetChat (灵犀宠语) / 13. 公益与寻宠 / Welfare & Lost-Pet
-- ============================================================
-- Version: 4.0.0
-- Created: 2026-06-17
...
CREATE TABLE public.t_rescue_request (
    ...
    CONSTRAINT fk_t_rescue_stat  FOREIGN KEY (f_status_user) REFERENCES public.t_status(f_id)    ON DELETE NO ACTION,
...
COMMENT ON COLUMN public.t_rescue_request.f_status_user   IS 'FK -> public.t_status(f_id) | defined in 01_enums.sql | 软删';
...
CREATE TABLE public.t_adoption (
    ...
    CONSTRAINT fk_t_adoption_stat  FOREIGN KEY (f_status_user)      REFERENCES public.t_status(f_id)          ON DELETE NO ACTION,
...
COMMENT ON COLUMN public.t_adoption.f_status_user      IS 'FK -> public.t_status(f_id) | defined in 01_enums.sql | 软删';
...
CREATE TABLE public.t_volunteer (
    ...
    CONSTRAINT fk_t_volunteer_stat  FOREIGN KEY (f_status_user)       REFERENCES public.t_status(f_id)          ON DELETE NO ACTION,
...
COMMENT ON COLUMN public.t_volunteer.f_status_user       IS 'FK -> public.t_status(f_id) | defined in 01_enums.sql | 软删';
...
CREATE TABLE public.t_donation (
    ...
    CONSTRAINT fk_t_donation_stat   FOREIGN KEY (f_status_user)    REFERENCES public.t_status(f_id)          ON DELETE NO ACTION,
...
CREATE TABLE public.t_record_lost_pet (
    ...
    CONSTRAINT fk_t_lost_stat  FOREIGN KEY (f_status_user) REFERENCES public.t_status(f_id)  ON DELETE NO ACTION,
    CONSTRAINT ck_t_lost_phone CHECK (f_contact_phone ~ '^[0-9+\-\s()]{5,32}$'),
    CONSTRAINT ck_t_lost_reward CHECK (f_reward_amount >= 0)
);
ALTER TABLE public.t_record_lost_pet ADD COLUMN IF NOT EXISTS f_status_lost INTEGER NOT NULL DEFAULT 1;
...
COMMENT ON COLUMN public.t_record_lost_pet.f_status_user     IS 'FK -> public.t_status(f_id) | defined in 01_enums.sql | 软删';
