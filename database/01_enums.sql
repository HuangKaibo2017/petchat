-- ============================================================
-- PetChat (灵犀宠语) / 1. 基础枚举层 / Foundation Enums
-- ============================================================
-- Version: 4.0.0
-- Created: 2026-06-17
...
--
-- 依赖:
--   00_extensions.sql (pgroonga / pgcrypto / uuid-ossp)
--
-- 被本文件引用的脚本 (下游):
--   02_rbac_users.sql            -> t_lang / t_status
--   03_pet_profile.sql           -> t_pet_type / t_pet_breed / t_gender / t_photo_type / t_status / t_lang
--   04_ai_reports.sql            -> t_report_type / t_risk_level / t_status / t_lang
--   05_chat_comments.sql         -> t_session_status / t_status / t_lang
--   06_share_interpretation.sql  -> t_share_type / t_share_medium / t_status / t_lang
--   07_cms.sql                   -> t_status / t_lang
--   08_subscription.sql          -> t_plan_type / t_payment_status / t_status
...
--   1. 所有 JSONB i18n 枚举统一为: BIGINT IDENTITY PK, f_name/f_desc JSONB, f_search_en TSVECTOR
--   2. 业务状态/类型枚举统一为: BIGINT IDENTITY PK + 业务语义 f_code (UNIQUE)
--   3. 哨兵记录: 业务状态枚举 f_id = -1 = NOT-SET; t_lang 不使用哨兵
--   4. 通用用户态: 1=active 2=disabled 3=deleted 4=archived 5=pending (在 t_status 内统一)
--   5. CJK 全文搜索: Supabase Cloud 用 pgroonga; 自建用 zhparser / pg_bigm (索引见 99_indexes_views.sql)
-- ============================================================
...
-- ============================================================
-- 通用模板:
--   f_id    BIGINT IDENTITY PK
--   f_name  JSONB NOT NULL DEFAULT '{}'  -- {"zh-CN":"..","en-US":"..","ja-JP":".."}
--   f_desc  JSONB NOT NULL DEFAULT '{}'
--   f_order INT, f_is_active BOOL
--   f_search_en TSVECTOR (英语全文搜索, GIN 索引见 99_indexes_views.sql)
--   业务字段: 按需, 一切引用 f_id
...
CREATE TABLE public.t_pet_type (
    ...
    f_search_en TSVECTOR GENERATED ALWAYS AS (
        to_tsvector('english',
            coalesce(f_name->>'en-US', '') || ' ' || coalesce(f_desc->>'en-US', ''))
    ) STORED,
    f_order     INTEGER NOT NULL DEFAULT 0,
    f_is_active BOOLEAN NOT NULL DEFAULT true,
...
COMMENT ON COLUMN public.t_pet_type.f_search_en IS '英语全文搜索 tsvector (GIN 索引见 99_indexes_views.sql: idx_t_pet_type_search_en)';
COMMENT ON COLUMN public.t_pet_type.f_order     IS '排序权重, 小→前';
COMMENT ON COLUMN public.t_pet_type.f_is_active IS '启用开关; 关闭后业务表不再展示';
...
COMMENT ON COLUMN public.t_pet_breed.f_search_en   IS '英语全文搜索 tsvector (GIN 索引见 99_indexes_views.sql: idx_t_pet_breed_search_en)';
COMMENT ON COLUMN public.t_pet_breed.f_order       IS '排序权重';
COMMENT ON COLUMN public.t_pet_breed.f_is_active   IS '启用开关';
...
CREATE TABLE public.t_gender (
    ...
    f_desc      JSONB NOT NULL DEFAULT '{}'::jsonb,
    f_search_en TSVECTOR GENERATED ALWAYS AS (
        to_tsvector('english', coalesce(f_name->>'en-US', ''))
    ) STORED,
    f_order     INTEGER NOT NULL DEFAULT 0,
    f_is_active BOOLEAN NOT NULL DEFAULT true,
...
COMMENT ON COLUMN public.t_gender.f_search_en IS '英语全文搜索 tsvector';
COMMENT ON COLUMN public.t_gender.f_order     IS '排序权重';
COMMENT ON COLUMN public.t_gender.f_is_active IS '启用开关';
...
COMMENT ON COLUMN public.t_photo_type.f_search_en IS '英语全文搜索 tsvector';
COMMENT ON COLUMN public.t_photo_type.f_order     IS '排序权重';
COMMENT ON COLUMN public.t_photo_type.f_is_active IS '启用开关';
...
COMMENT ON COLUMN public.t_report_type.f_search_en IS '英语全文搜索 tsvector (GIN 索引见 99_indexes_views.sql)';
COMMENT ON COLUMN public.t_report_type.f_order     IS '排序权重';
COMMENT ON COLUMN public.t_report_type.f_is_active IS '启用开关';
...
COMMENT ON COLUMN public.t_risk_level.f_search_en IS '英语全文搜索 tsvector';
COMMENT ON COLUMN public.t_risk_level.f_order     IS '排序权重';
COMMENT ON COLUMN public.t_risk_level.f_is_active IS '启用开关';
...
COMMENT ON COLUMN public.t_personality_tag.f_search_en IS '英语全文搜索 tsvector (GIN 索引见 99_indexes_views.sql)';
COMMENT ON COLUMN public.t_personality_tag.f_order     IS '排序权重';
COMMENT ON COLUMN public.t_personality_tag.f_is_active IS '启用开关';
...
COMMENT ON TABLE  public.t_status IS '通用用户态: 1=active 2=disabled 3=deleted 4=archived 5=pending; 软删统一通过 f_status_user=3 表达; 几乎所有业务表的 f_status_user 都引用此表';
COMMENT ON COLUMN public.t_status.f_id        IS '主键; 哨兵: -1 = NOT-SET; 业务约定值: 1=active 2=disabled 3=deleted 4=archived 5=pending';
...
COMMENT ON COLUMN public.t_status.f_search_en IS '英语全文搜索 tsvector (GIN 索引见 99_indexes_views.sql)';
COMMENT ON COLUMN public.t_status.f_order     IS '排序权重';
COMMENT ON COLUMN public.t_status.f_is_active IS '启用开关';
...
COMMENT ON COLUMN public.t_plan_type.f_search_en IS '英语全文搜索 tsvector';
COMMENT ON COLUMN public.t_plan_type.f_order     IS '排序权重';
COMMENT ON COLUMN public.t_plan_type.f_is_active IS '启用开关';


-- ============================================================
-- 1.3 业务域状态/类型枚举表 (10 张, 全部含 -1 = NOT-SET; 业务表 f_status_xxx INT4 引用)
-- ============================================================
-- 结构相同: BIGINT IDENTITY PK, f_code 业务语义代码 (UNIQUE), JSONB i18n
...
COMMENT ON COLUMN public.t_payment_status.f_is_active IS '启用开关';
...
COMMENT ON COLUMN public.t_shipping_status.f_is_active IS '启用开关';


-- 1.3.3 聊天会话状态 / Session Status
CREATE TABLE public.t_session_status (
    f_id        BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    f_code      VARCHAR(32) NOT NULL UNIQUE,
    f_name      JSONB NOT NULL DEFAULT '{}'::jsonb,
    f_desc      JSONB NOT NULL DEFAULT '{}'::jsonb,
    f_order     INTEGER NOT NULL DEFAULT 0,
    f_is_active BOOLEAN NOT NULL DEFAULT true,
    CONSTRAINT ck_t_session_status_name_is_object CHECK (jsonb_typeof(f_name) = 'object')
);
COMMENT ON TABLE  public.t_session_status IS '聊天会话状态 (active/closed/archived/...)';
COMMENT ON COLUMN public.t_session_status.f_id        IS '主键; 哨兵: -1 = NOT-SET';
COMMENT ON COLUMN public.t_session_status.f_code      IS '业务语义代码, e.g. active, closed, archived | UNIQUE';
COMMENT ON COLUMN public.t_session_status.f_name      IS '多语言名称 | 引用方: t_chat_session.f_status_session (in 05_chat_comments.sql)';
COMMENT ON COLUMN public.t_session_status.f_desc      IS '多语言描述';
COMMENT ON COLUMN public.t_session_status.f_order     IS '排序权重';
COMMENT ON COLUMN public.t_session_status.f_is_active IS '启用开关';


-- 1.3.4 设备同步状态 / Sync Status
...
COMMENT ON COLUMN public.t_sync_status.f_is_active IS '启用开关';
...
COMMENT ON COLUMN public.t_inventory_status.f_is_active IS '启用开关';
...
COMMENT ON COLUMN public.t_inventory_serial_status.f_is_active IS '启用开关';
...
COMMENT ON COLUMN public.t_adoption_type.f_is_active IS '启用开关';
...
COMMENT ON COLUMN public.t_volunteer_type.f_is_active IS '启用开关';
...
COMMENT ON COLUMN public.t_share_type.f_is_active IS '启用开关';


-- 1.3.10 分享介质 / Share Medium
CREATE TABLE public.t_share_medium (
    f_id        BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    f_code      VARCHAR(32) NOT NULL UNIQUE,
    f_name      JSONB NOT NULL DEFAULT '{}'::jsonb,
    f_desc      JSONB NOT NULL DEFAULT '{}'::jsonb,
    f_order     INTEGER NOT NULL DEFAULT 0,
    f_is_active BOOLEAN NOT NULL DEFAULT true,
    CONSTRAINT ck_t_share_medium_name_is_object CHECK (jsonb_typeof(f_name) = 'object')
);
COMMENT ON TABLE  public.t_share_medium IS '分享介质 (wechat/moments/qq/link/in_app/...)';
COMMENT ON COLUMN public.t_share_medium.f_id        IS '主键; 哨兵: -1 = NOT-SET';
COMMENT ON COLUMN public.t_share_medium.f_code      IS '业务语义代码, e.g. wechat, moments, qq, link, in_app | UNIQUE';
COMMENT ON COLUMN public.t_share_medium.f_name      IS '多语言名称 | 引用方: t_share_record.f_share_medium_id (in 06_share_interpretation.sql)';
COMMENT ON COLUMN public.t_share_medium.f_desc      IS '多语言描述';
COMMENT ON COLUMN public.t_share_medium.f_order     IS '排序权重';
COMMENT ON COLUMN public.t_share_medium.f_is_active IS '启用开关';
